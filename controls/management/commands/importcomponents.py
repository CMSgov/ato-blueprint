...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--format", metavar="format", nargs="?", default="oscal", help="File format"
        )
        parser.add_argument(
            "--path",
            metavar="dir_or_pdf",
            nargs="?",
            default="local/export/components",
            help="The directory path containing component files to import.",
        )
        parser.add_argument(
            "--importname",
            metavar="importname",
            nargs="?",
            default="Batch component import",
            help="Name to identify the batch import",
        )
        ...
        if FORMAT == "oscal":
            ...
            for path in pathlist:
                ...
                with open(path_in_str) as f:
                    ...
                    result = ComponentImporter().import_components_as_json(
                        IMPORT_NAME,
                        oscal_component_json,
                        existing_import_record=True,
                        stopinvalid=STOPINVALID,
                    )
