...
class PoamForm(ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["poam_group"].widget.attrs[
            "placeholder"
        ] = "e.g., Epic 23, Log Fixes"
        self.fields["risk_rating_original"].widget.attrs[
            "placeholder"
        ] = "e.g., Low, Moderate, or High"
        self.fields["risk_rating_adjusted"].widget.attrs[
            "placeholder"
        ] = "e.g., Low, Moderate, or High"
        ...
        self.fields["scheduled_completion_date"].widget.attrs[
            "placeholder"
        ] = "YYYY-MM-DD"
    ...
    def clean(self):
        ...
        if Element.objects.filter(name__iexact=cd["name"]).exists():
            raise ValidationError(
                "Component (aka Element) name {} not available.".format(cd["name"])
            )
        return cd
...
class ImportOSCALComponentForm(forms.Form):
    ...
    file = forms.FileField(
        label="Select OSCAL file (.json)",
        widget=forms.FileInput(
            attrs={"onchange": "fillJSONContent(this);", "accept": "application/json"}
        ),
        required=False,
    )
    json_content = forms.CharField(label="OSCAL (JSON)", widget=forms.Textarea())
    import_name = forms.CharField(
        label="Import File Name", widget=forms.HiddenInput(), required=False
    )
...
class ImportProjectForm(forms.Form):
    ...
    file = forms.FileField(
        label="Select project file (.json)",
        widget=forms.FileInput(
            attrs={
    ...
    json_content = forms.CharField(
        ...
        help_text="The JSON necessary for importing a project.",
    )
...
class DeploymentForm(ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.initial["inventory_items"] = json.dumps(
            self.instance.inventory_items, indent=4, sort_keys=True
        )
    ...
    inventory_items = forms.CharField(
        label="Inventory items (JSON)",
        required=False,
        widget=forms.Textarea(),
...
class SystemAssessmentResultForm(ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["system"].widget = forms.HiddenInput()
        # Display pretty JSON in JSONfield's text area
        self.initial["assessment_results"] = json.dumps(
            self.instance.assessment_results, indent=4, sort_keys=True
        )
    ...
    assessment_results = forms.CharField(
        label="System assessment result items (JSON)",
        required=False,
        widget=forms.Textarea(),
