...
def load_catalog_data_800_171_fixed(apps, schema_editor):
    ...
    catalog_files = [
        ...
        if file.startswith("NIST_SP-800-171_rev1_catalog")
    ]
    ...
    for cf in catalog_files:
        ...
        catalog, created = CatalogData.objects.get_or_create(catalog_key=catalog_key)
        if created:
...
def update_smt_800_171(apps, schema_editor):
    ...
    smts = Statement.objects.filter(sid_class=catalog_key, sid__icontains="3.").only(
        "sid"
    )
    # update sid
    for smt in smts:
