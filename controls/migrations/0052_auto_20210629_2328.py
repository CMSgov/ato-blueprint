...
def update_statement_type_to_enum_name(apps, schema_editor):
    ...
    apps.get_model("controls", "Statement").objects.filter(
        statement_type=StatementTypeEnum.ASSESSMENT_RESULT.value
    ).update(statement_type=StatementTypeEnum.ASSESSMENT_RESULT.name)

    apps.get_model("controls", "Statement").objects.filter(
        statement_type=StatementTypeEnum.POAM.value
    ).update(statement_type=StatementTypeEnum.POAM.name)

    apps.get_model("controls", "Statement").objects.filter(
        statement_type=StatementTypeEnum.SECURITY_SENSITIVITY_LEVEL.value
    ).update(statement_type=StatementTypeEnum.SECURITY_SENSITIVITY_LEVEL.name)

    apps.get_model("controls", "Statement").objects.filter(
        statement_type=StatementTypeEnum.SECURITY_IMPACT_LEVEL.value
...
class Migration(migrations.Migration):
    ...
    operations = [migrations.RunPython(update_statement_type_to_enum_name)]
