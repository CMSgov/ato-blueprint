...
def get_controls_addressed_count(project):
    stat = (
        ElementControl.objects.filter(element_id=project.system.root_element)
        .values("status")
...
def get_control_compliance_stats(project):
    ...
    total_controls_count = ElementControl.objects.filter(
        element_id=project.system.root_element
    ).count()
